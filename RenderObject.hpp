...
struct Text {
  ...
  Text(FontDescriptor font, std::string text, const IntVector &position, const SDL_Color &color) : font(std::move(
          font)), text(std::move(text)), position(position), color(color) {}
};
